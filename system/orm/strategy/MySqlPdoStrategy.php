...
class MySqlPdoStrategy {
    ...
    /**
     * Realiza a operaзгo de select no banco de dados
    ...
    /**
     * Lista uma collection de objetos sem formataзгo
     * @param type $where
     * @param type $objectCollection
     * @return boolean
     */
    ...
    public function salvar($dados, $objectResult = null, array $exception = null) {
        if (is_array($dados)) {
            ...
            $result[0] = false;
            $result[1] = $dados;
            $result[2]['post'] = 'Nгo pode ser salvo (Para salvar precisa passar um objeto ou um array)';
            return $result;
        }
    ...
    /**
     * Deletar uma coleзгo de dados na base de dados
     * @param type $id
     * @return type
     */
     ...
     * @param type $array
     *  null : nгo carrega, false: carrega id, true carrega obj
     * @param type $objectCollection
     * @return boolean
     */
    public function loadObject($array, $objectCollection = null, $exception = null) {
        ...
        foreach ($this->propAtributos as $atributo => $propriedades) {
            ...
            if ($colmap !== false) {
                ...
                # Se existe um colmap e ele й diferente de '' e null
                if (isset($array[$colmap]) && $array[$colmap] !== '' && $array[$colmap] !== null) {
                ...
                if (isset($propriedades['OneToMany'])) {
                    ...
                    if ($result !== false) {
                        ...
                        $collection = array();
                        $idColmapStrategy = (isset($propriedades['ManyToMany']['coluna'])) ? $propriedades['ManyToMany']['coluna'] : $strategy->getIdColmap() ;
                        foreach ($result as $array) {
                            $collection[] = $strategy->obterPorId($array[$idColmapStrategy], false);
                        }
                        unset($idColmapStrategy);
                ...
                if (isset($propriedades['OneToMany'])) {
                    ...
                    if ($result !== false) {
                        ...
                        $idColmapStrategy = (isset($propriedades['ManyToMany']['coluna'])) ? $propriedades['ManyToMany']['coluna'] : $strategy->getIdColmap() ;
                        foreach ($result as $array) {
                            $collection[] = $array[$idColmapStrategy];
                        }
    ...
    private function inserir($array, $objectResult = null) {
        ...
        if (!$dados[0]) {
            $result[0] = false;
            $result[2]['null'] = "Nгo existe dados a ser inserido";
            return $result;
        }
        ...
        if (isset($inserir['entity'])) {
            ...
            if (isset($inserir['ManyToMany'])) {
                ...
                foreach ($inserir['ManyToMany'] as $atributo => $values) {
                    ...
                    $id_relationship = (isset($relationship['coluna'])) ? $relationship['coluna'] : $reflectionRelationship->getPropAnnotations('id', '@Colmap');
