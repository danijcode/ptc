...
class SecurityController extends TPtc {
    ...
    public function logon() {
        ...
        # Adionar CSS
        $this->HTML->addCss(PATH_TEMPLATE_URL . "css/login.css");
    ...
    public function logar(){
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            $objProfessor = $this->logic->obter("email = '{$email}' AND senha = '{$senha}'");
            
            if(is_object($objProfessor)){
                $security = SecurityHelper::getInstancia();
                $security->iniciarSessao($objProfessor);
                $this->REDIRECT->goToController('Administrator');
            }else{
                $this->REDIRECT->setUrlParameter('feedback','1');
                $this->REDIRECT->goToControllerAction('Security','logon');
            }
    ...
    public function recuperar() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            $email = FormatHelper::unformatCPF($_POST['email']);
            $objProfessorLogic = new ProfessorLogic();
            $objUsuario = $objProfessorLogic->obter("email = '{$email}'");
            ...
            if ($token->getDataValidade() < time() || $token->getTentativas() === 3) {
                ...
                if ($token->getSenhaAutenticacao() == md5($_POST['senhaAutenticacao'])) {
                    ...
                    if (!$salvar) {
                        ...
                        $objVUsuarioLogic = new VUsuarioLogic();
